...
- **AI 整理**：默认调用本机 Ollama 生成内容摘要，并提取 2–5 个突出重点的标签；模型不可用时自动采用本地规则。
...
扩展只将用户主动选择保存的当前页面内容发送到 `127.0.0.1` 本机服务。本机服务默认使用本机 Ollama 完成 AI 整理，再通过用户本人授权的 `lark-cli` 写入用户自己的飞书空间。认证信息由 `lark-cli` 管理，扩展及本项目均不保存飞书密码、应用密钥或访问令牌。
